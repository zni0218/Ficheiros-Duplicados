...
Teste simples de indexação para image_phash.
...
# ✅ IMPORT DO INDEXING
from indexing.image_phash import compute_index_image_phash
...
def build_image_index(base: Path, out: Path, debug=False):
    ...
    csv_path = out / "image_phash_test_index.csv"
    ...
    raw_index = compute_index_image_phash(files, debug=debug)

    if not raw_index:
        print("[ERROR] Nenhuma imagem indexada")
        return
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Teste image_phash index"
    )
