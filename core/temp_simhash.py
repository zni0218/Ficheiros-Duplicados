...
Teste simples de indexação para text_simhash.
...
# ✅ IMPORT DO INDEXING
from indexing.text_simhash import compute_index_text_simhash
...
def build_simhash_index(base: Path, out: Path, debug=False):
    ...
    csv_path = out / "text_simhash_test_index.csv"
    ...
    raw_index = compute_index_text_simhash(files, debug=debug)

    if not raw_index:
        print("[ERROR] Nenhum ficheiro texto indexado")
        return
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Teste text_simhash index"
    )
